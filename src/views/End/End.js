...
const End = ()=> {
  ...
  useEffect(()=> {
    document.title = `Results | Color riddle`
  ...
  const handleCopy = ()=> {
    playSound('click')
    const introText = `I got ${correct}/${total} color riddles correct in only 🕒 ${totalTime}s!`
    const levelsText = data.map(levelResult=> {
        const { level, status, score } = levelResult
        const result = status === 'correct' ? '🟢' : '🔴'
        return `${result} Level ${level} - 🕒 ${score}s`
      })
      .join('\n')
  ...
  return (
    <div className={css.main}>
      ...
      <p className={css.intro}>
        <code>
          You answered correctly <span data-color="green">{correct}</span>/{total} riddles
          <br />
          Your total time is of <span data-color="pink">🕒 {totalTime}s</span>
        </code>
      </p>
      ...
      {type === 'quick' && (
        <>
          ...
          {isCopied && (
            <code className={css.copied}>Copied to your clipboard! Share the love!</code>
          )}
        </>
      )}
      <button 
